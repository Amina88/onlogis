...
$i=0;
	...
	$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  DT.Numero_Compte='$admin[0]' and J.Date= CURDATE( ) AND DT.etat=0")or die(mysql_error());;
$test_existe=0;
...
$total_Credit=0;
$sold=0;
$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  DT.Numero_Compte='$admin[0]'  AND DT.etat=0");
while($jrnle=mysql_fetch_array($jrnl)){
...
// $Fr=mysql_query("select compte from ");
?>	
...
<tr  style="background:#ececec;height:6px;"  > 
    				<td colspan="2"><font size="1"><?php echo $N2.'  '.$i; ?></td> 
...
$i=0;
	while($admin=mysql_fetch_array($comm2)){
		...
		$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  DT.Numero_Compte='$admin[0]' and J.Date>=$date1 AND DT.etat=0");
$test_existe=0;
	while($jrnle=mysql_fetch_array($jrnl)){
	$test_existe=1;
	}
...
?>
						  ...
						  <div class="portlet box yellow">
                           <div class="portlet-title">
							...
							<form action="Balance_Compte.php" id="form_sample_2" class="form-horizontal" method="post" target=_blank>
								<div class="form-body">
