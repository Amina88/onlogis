...
$MN=mysql_fetch_array($curr);
	...
	$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  J.Partenaire='$admin[0]' and J.Date= CURDATE( ) AND DT.etat=0")or die(mysql_error());;
$test_existe=0;
	...
	?>
	<tr   > 
    				<td colspan='5'><font size="1"><b><?php echo $admin[0]; ?></b></td> 
...
?>	
				<tr   style="background:#fcfcfc;height:6px;"> 
    				<td><font size="1"><?php echo $cpt[0]; ?></td> 
    				<td><font size="1"><?php echo $cpt_comptable; ?></td> 
    				<td><font size="1"><?php echo number_format($total_Credit, 2, ',', ' ').' '.$MN[0]; ?></td> 
    				<td><font size="1"><?php echo number_format($total_Debit, 2, ',', ' ').' '.$MN[0]; ?></td> 
    				
					<td><font size="1"><?php echo number_format($sold, 2, ',', ' ').' '.$MN[0]; ?></td> 
...
<tr  style="background:#ececec;height:6px;"  > 
    				<td colspan="2"><font size="1"><?php echo $N2; ?></td> 
...
$MN=mysql_fetch_array($curr);
	...
	$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  J.Partenaire='$admin[0]' and J.Date>=$date1 AND DT.etat=0")or die(mysql_error());;
$test_existe=0;
	while($jrnle=mysql_fetch_array($jrnl)){
	$test_existe=1;
	}
...
?>
						  ...
						  <div class="portlet box yellow">
                           <div class="portlet-title">
							...
							<form action="Balance_Tier.php" id="form_sample_2" class="form-horizontal" method="post" target=_blank>
								<div class="form-body">
...
<?php 
$tier=array();
$Fr=mysql_query("select * from supplier");
$CL=mysql_query("select * from custemer");
while($Four=mysql_fetch_array($Fr)){ 
if(!in_array("$Four[0]",$tier)){
 $tier[]=$Four[0];
...
foreach($tier as $TR){
?>
<option  value="<?php echo $TR;?>"><?php echo $TR;?></option>
<?php }?>
</select></div>
