...
$MN=mysql_fetch_array($curr);
	...
	$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  J.Partenaire='$admin[0]' and J.Date= CURDATE( ) AND DT.etat=0");
$test_existe=0;
...
$MN=mysql_fetch_array($curr);
	while($admin=mysql_fetch_array($comm2)){
		$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  J.Partenaire='$admin[0]' and J.Date>=$date1 AND DT.etat=0");
$test_existe=0;
	while($jrnle=mysql_fetch_array($jrnl)){
	$test_existe=1;
	}
...
$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and  J.Partenaire='$admin[0]' and J.Date>=$date1 AND DT.etat=0");
	...
	if(($jrnle[1]=='Achat' and $jrnle[5]!=0) or $jrnle[1]=='Paiement'){
?>
				...
				<tr style="background:#fcfcfc;height:6px;" > 
    				...
    				<td><font size="1"><?php echo number_format($jrnle[5], 2, ',', ' ').' '.$MN[0]; ?></td> 
    				<td><font size="1"><?php echo number_format($jrnle[4], 2, ',', ' ').' '.$MN[0]; ?></td> 
					<td><font size="1"><?php echo number_format($jrnle[4]-$jrnle[5], 2, ',', ' ').' '.$MN[0]; ?></td> 
...
$sold=$sold+$d;
 	...
 	if($jrnle[1]=='Vente' and $jrnle[4]!=0){
?>
...
?>
						  ...
						  <div class="portlet box yellow">
                           <div class="portlet-title">
							...
							<form action="GrandLivre_Tiers.php" id="form_sample_2" class="form-horizontal" method="post" target=_blank>
								<div class="form-body">
...
foreach($tier as $TR){
?>
<option selected value="<?php echo $TR;?>"><?php echo $TR;?></option>
<?php }?>
</select></div></div>
									<div class="form-group">
