...
$pdf->fact_dev( "Grand-livre des tiers", "");
$date=date("d/m/Y");
...
$comm2=mysql_query("select DISTINCT `Partenaire`  from journal where Partenaire like '$Tier'  ")or die(mysql_error());
	...
	$data[] =array(utf8_decode("Start"),utf8_decode("$admin[0]"),utf8_decode(""));
...
$total_Credit=$jrnle[5]+$total_Credit;
	$tt=$sold+$tt;
        ...
        $data[] =array(utf8_decode("$jrnle[0]"),utf8_decode("$jrnle[1]"),utf8_decode("$jrnle[2]"),utf8_decode("$jrnle[3]"),number_format($jrnle[5], 2, ',', ' ').' '.$MN1[0],number_format($jrnle[4], 2, ',', ' ').' '.$MN1[0],number_format($sold, 2, ',', ' ').' '.$MN1[0]);
