...
include('fpdf17/font/courier.php');include('fpdf17/font/helvetica.php');include('fpdf17/font/times.php');
...
function addtotalfact($tva,$tt,$m,$net)
{
	...
	$this->Cell(10,4, "TOTAL NET      ".$tva."  ".$m, 0, 0, "L");
	$this->Ln();
	$this->Cell(10,4, "TOTAL TVA      ".$tt. "  ".$m, 0, 0, "L");
	$this->Ln();
	$this->Cell(10,4, "NET A PAYER   ".$net."  ".$m, 0, 0, "L");
}
function addtotalfactT($tva,$tt,$m,$net)
{
	$r1  = $this->w - 80;
	$r2  = $r1 + 75;
	$y1  = $this->h - 70;
	...
	$this->Cell(10,4, "TOTAL NET", 0, 1, "L");
...
function Header()
	...
	include('Connection.php');
$Reff=$data['id_offre'];
$MN=mysql_query("select TVA from  element_offre where Reference='$Reff'");
...
$nln=0;
$push_right = 0;
$nln1=$this->NbLines(50,$data['desig']);
$nln2=$this->NbLines(20,$data['qte']);
$nln3=$this->NbLines(29,$data['pu']);
$nln4=$this->NbLines(29,$data['net']);
$nln5=$this->NbLines(19,$data['tva']);
$nln6=$this->NbLines(30,$data['tt']);
 $nln=max($nln1,$nln2,$nln3,$nln4,$nln5,$nln6);
$this->MultiCell($w =10,5*$nln,$i,1,'LT',1);

$push_right += $w;
$this->SetXY($x + $push_right, $y);
$this->MultiCell($w = 50,5,utf8_decode($data['desig']),1,'LT',1);

$push_right += $w;
$this->SetXY($x + $push_right, $y);
$this->MultiCell($w = 20,5*$nln,$data['qte'],1,'C',1);

$push_right += $w;
$this->SetXY($x + $push_right, $y);

$this->MultiCell($w = 29,5*$nln,$data['pu']." ".$data['monnaie'],1,'C',1);

$push_right += $w;
$this->SetXY($x + $push_right, $y);

$this->MultiCell($w = 29,5*$nln,$data['net']." ".$data['monnaie'],1,'C',1);

$push_right += $w;
$this->SetXY($x + $push_right, $y);

$this->MultiCell($w = 29,5*$nln,"($MN1[0]%)  ".$data['tva']." ".$data['monnaie'],1,'C',1);

$push_right += $w;
$this->SetXY($x + $push_right, $y);

$this->MultiCell(30,5*$nln,$data['tt']." ".$data['monnaie'],1,'C',1);
