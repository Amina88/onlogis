...
$pdf->fact_dev( "Balance des comptes", "");
$date=date("d/m/Y");
...
$MN=mysql_query("select * from currency where Monnaie_utliser='1'");
$MN1=mysql_fetch_array($MN);
$comm2=mysql_query("select DISTINCT `Numero_Compte` ,`Compte` from detaile_journal where etat=0 and Numero_Compte like '%$Tier%' order by Numero_Compte desc   ")or die(mysql_error());
    // Lecture des lignes du fichier
	 $data = array();
	while($admin=mysql_fetch_array($comm2)){
	 $test=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and DT.Numero_Compte='$admin[0]'  and J.Date BETWEEN '$DD' AND '$DF'");
$total_Debit=0;
$total_Credit=0;
$tt=0;

$jrnl=mysql_query("select Date,journal,J.id,Libelle,Debit,Credit from detaile_journal DT ,journal J where DT.id_journal=J.id and DT.Numero_Compte='$admin[0]' and  DT.etat=0 and J.Date BETWEEN '$DD' AND '$DF'");
...
$total_Credit=$jrnle[5]+$total_Credit;
	$tt=$sold+$tt;
        ...
        $data[] =array(utf8_decode("$admin[0]"),utf8_decode("$admin[1]"),number_format($total_Credit, 2, ',', ' ').' '.$MN1[0],number_format($total_Debit, 2, ',', ' ').' '.$MN1[0],number_format($sold, 2, ',', ' ').' '.$MN1[0]);
