...
$doc->load($file_Menu); 
$Menu = $doc->getElementsByTagName("finalstatementFournisseur");
...
$ic="select REFERENCE as code,date_commande as date_lancement,date_echeance as date_paiement,DATE(date_commande) -  DATE(date_echeance)  as echue,type_de_monnaie as monnaie,valeur_payer as valeur_payer,dossiers as etat  from purchase where fournisseur='$NomSoc' and etat_paiement=0 and etat_commande=1 UNION select id_facture as code,date_lancement as date_lancement,Date_paiment as date_paiement,Jours_echue as echue,Monnaie_secondaire as monnaie,valeur_payer as valeur_payer, draft as etat from invoice where client='$NomSoc' and etat_payement=0 and draft=1";
...
$req_provider = mysql_query("select * from supplier where Nom_Soc='$NomSoc'");
while($admin= mysql_fetch_array($req_provider)){
$c_nom = $admin[0];$c_mail = $admin[5];$c_t1= $admin[6];$c_t2=$admin[7];$c_adr=$admin[15];
}


$tc=mysql_query("select * from purchase where fournisseur='$NomSoc' and etat_paiement=0 and etat_commande=1") or die(mysql_error());
$monitab=array();
$monitab2=array();
$totaltab=array();
$totaltab2=array();
 while($ics=mysql_fetch_array($tc)){
 $aLongTimeAgo = strtotime('$ics[4]');
$armageddon   = strtotime('$ics[5]');
$Jours_echue=$armageddon-$armageddon;
  if($Jours_echue>0){
$f=$ics['reference'];
$tfs=mysql_query("select * from vuepurchasetotale where BonCommande='$f'");
$tt=mysql_fetch_array($tfs);
if($tt['TOTAL_COM']<0){
$rest=$tt['TOTAL_COM']+$ics['valeur_payer'] ;
}else{
$rest=$tt['TOTAL_COM']-$ics['valeur_payer'] ;
}
 $mv=$ics['type_de_monnaie'];
 if (!in_array("$mv",$monitab)) {
$monitab["$mv"]=$ics['type_de_monnaie'];
$totaltab["$mv"]=$rest;
...
$monitab2["$mv"]=$ics['type_de_monnaie'];
$totaltab2["$mv"]=$rest*-1;
